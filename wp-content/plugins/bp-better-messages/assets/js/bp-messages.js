...
(function ($) {
    ...
    function initialInit() {
        ...
        bpMessagesWrap.on('submit', '.new-message form', function (event) {
            ...
            $(document).trigger("bp-better-messages-new-thread-start");

            $.post(BP_Messages.ajaxUrl, data, function (data) {
                if (data.result) {
                    $(document).trigger("bp-better-messages-new-thread-created");
                    ajaxRefresh(BP_Messages.threadUrl + data['result'], container);
                } else {
                    $(document).trigger("bp-better-messages-new-thread-error");
                ...
                $(document).trigger("bp-better-messages-new-thread-end");
            }).fail(function() {
                form.closest('.bp-messages-wrap').find('.preloader').hide();
    ...
    function reInit() {
        ...
        function initializeEmojiArea(selector){
            var emojioneareas = $(selector).BPemojioneArea({
                tones: true,
                tonesStyle: 'bullet',
            ...
            emojioneareas.each(function(){
                var emojionearea = $(this);
                ...
                if( typeof emojionearea[0] !== 'undefined' ) {
                    ...
                    function afterAreaLoaded(){
                        ...
                        if( areaExists ) {
                            ...
                            var media_editor = new BPBM_MediumEditor(document.querySelectorAll('.bp-emojionearea-editor'), opts);
        ...
        if( typeof $.fn.BPBMoverlayScrollbars === 'function' ) {
            ...
            $(document).trigger("bp-better-messages-init-scrollers");
        }
    ...
    function ajaxRefresh(url, container) {
        ...
        if( isInCall ){
            if ( confirm(BP_Messages['strings']['you_are_in_call']) === false ) {
                ...
                BPBMJQ(document).trigger('bpbm-end-group-call');
            }
        }
        ...
        $(document).trigger("bp-better-messages-ajax-refresh-start");
        ...
        $.ajax({
            ...
            success: function (json) {
                ...
                $(document).trigger("bp-better-messages-ajax-refresh-end");
            }
        });
    }
    ...
    function sortOnlineFriends(){
        var friendsWidgets = $('.bp-better-messages-list .tabs-content .friends, .bpbm-friends-list');
