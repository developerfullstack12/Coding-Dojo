...
class Learndash_Paidmemberships {
	...
	public static function define_constants() 
	{
		...
		if ( ! defined( 'LEARNDASH_PMP_VERSION' ) ) {
			define( 'LEARNDASH_PMP_VERSION', '1.3.2' ); 
		}
	...
	public static function hooks()
	{
		...
		add_action( 'pmpro_after_change_membership_level', array( 'Learndash_Paidmemberships', 'after_change_membership_level' ), 10, 3 );
	 ...
	 * @return void
	 */
	public static function after_change_membership_level( $level_id, $user_id, $cancel_level ) 
	{
	...
	public static function cron_update_object_access()
	{
		...
		foreach ( $updates as $membership_id => $update ) {
			...
			$members = self::get_active_members( $membership_id, $update );
	...
	public static function cron_process_silent_object_enrollment() {
	    ...
	    foreach ( $processed_queue as $key => $args ) {
	        if ( ! empty( $args ) ) {
	            $ld_objects = self::get_level_objects( $args['membership_id'] );

	            foreach ( $ld_objects as $object_id ) {
                    $post_type = get_post_type( $object_id );
	                if ( $args['action'] == 'enroll' ) {
	                    if ( $post_type == 'sfwd-courses' ) {
	                    	ld_update_course_access( $args['user_id'], $object_id );
	                    } elseif ( $post_type == 'groups' ) {
	                    	ld_update_group_access( $args['user_id'], $object_id );
	                    }
	                } elseif ( $args['action'] == 'unenroll' ) {
	                    if ( $post_type == 'sfwd-courses' ) {
	                    	ld_update_course_access( $args['user_id'], $object_id, true );
	                    } elseif ( $post_type == 'groups' ) {
	                    	ld_update_group_access( $args['user_id'], $object_id, true );
	                    }
	                }
	            }
	        }
	 ...
	 * 
	 * @param  int  $membership_id  ID of a membership level
	 * @param  int  $user_id 		ID of WP_User
	 * @param  boolean $remove  	True to remove course access|false otherwise
	 * @return void
	 */
	public static function update_object_access( $membership_id, $user_id, $remove = false ) 
	{
		$objects = Learndash_Paidmemberships::get_level_objects( $membership_id );

		if ( count( $objects ) >= self::get_min_courses_count_for_silent_course_enrollment() ) {
			$action = $remove ? 'unenroll' : 'enroll';
			self::enqueue_silent_object_enrollment( [ 'membership_id' => $membership_id, 'user_id' => $user_id, 'action' => $action ] );
		} else {
	 ...
	 * @return void
	 */
	public static function get_active_members( $membership_id, $data )
	{
		global $wpdb;
		...
		$members = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->pmpro_memberships_users} WHERE membership_id = %d AND status = 'active' LIMIT %d OFFSET %d", $membership_id, $per_batch, $offset ) );
